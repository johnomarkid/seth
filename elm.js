...
Elm.Html.CssHelpers.make = function (_elm) {
   ...
   var style = function (text) {
      return A3($Html.node,
      "style",
      _U.list([A2($Html$Attributes.property,"textContent",$Json$Encode.string(text)),A2($Html$Attributes.property,"type",$Json$Encode.string("text/css"))]),
      _U.list([]));
   };
   ...
   return _elm.Html.CssHelpers.values = {_op: _op,withNamespace: withNamespace,style: style,stylesheetLink: stylesheetLink};
};
Elm.Utils = Elm.Utils || {};
...
Elm.MyCss.make = function (_elm) {
   ...
   var titleFont = _U.list([$Css.fontSize($Css.em(2)),$Css.color($Css.hex("66ACFF"))]);
   var Page = {ctor: "Page"};
   var Panel = {ctor: "Panel"};
   var css = function (_p0) {
      return $Css.stylesheet(A2($Css$Namespace.namespace,"seth",_p0));
   }(_U.list([$Css$Elements.body(_U.list([]))
             ,$Css$Elements.input(_U.list([$Css.border($Css.px(0)),$Css.backgroundColor(A4($Css.rgba,0,0,0,0))]))
             ,A2(F2(function (x,y) {    return A2($Css._op["."],x,y);}),Panel,_U.list([$Css.backgroundColor($Css.hex("F8F8F8")),$Css.padding($Css.px(20))]))]));
   return _elm.MyCss.values = {_op: _op,Panel: Panel,Page: Page,css: css,titleFont: titleFont,primaryAccentColor: primaryAccentColor};
};
Elm.Education = Elm.Education || {};
Elm.Education.make = function (_elm) {
   "use strict";
   _elm.Education = _elm.Education || {};
   if (_elm.Education.values) return _elm.Education.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var _p2 = $Html$CssHelpers.withNamespace("seth");
   var id = _p2.id;
   var $class = _p2.$class;
   ...
   var educationRow = F2(function (address,item) {
      ...
      _U.list([$Html.text(A2($Basics._op["++"],"id: ",A2($Basics._op["++"],$Basics.toString(item.id),"   ")))
              ,A2($Html.input,
              _U.list([$Html$Attributes.value(item.school)
                      ,A3($Html$Events.on,
                      "input",
              ...
              _U.list([]))
              ,A2($Html.input,
              _U.list([$class(_U.list([$MyCss.HiddenInput]))
   ...
   var view = F2(function (address,items) {
      ...
      return A2($Html.div,
      _U.list([$class(_U.list([$MyCss.Panel]))]),
